...
Scale.prototype.mouseup =
  function mouseup (e)
  {
    ...
    if (d.left || d.top || d.right || d.bottom) return;
 
    // Scale in all directions.
    this.resize.adjust(Util.merge(this.block.geom, { x : -Infinity, d : { left   : true } }));
    this.resize.adjust(Util.merge(this.block.geom, { y : -Infinity, d : { top    : true } }));
    this.resize.adjust(Util.merge(this.block.geom, { r :  Infinity, d : { right  : true } }));
    this.resize.adjust(Util.merge(this.block.geom, { b :  Infinity, d : { bottom : true } }));
  };
